...
ovmesh::Reception authorized_packet(){auto p=packet();p.id=2;p.decoded.status=ovmesh::protocol::Status::decoded;p.decoded.classification="likely Meshtastic";ovmesh::protocol::AuthorizedContent a;a.profile_id="synthetic-profile";a.from=0x11111111;a.to=0xffffffff;a.port=1;a.content.kind="text";a.content.text="SYNTHETIC CONCENTRATOR CONTENT";p.decoded.authorized=a;return p;}
void create(const std::filesystem::path& path){const auto c=config();ovmesh::SessionStore store;store.create(path.string(),c,"synthetic-concentrator");store.append(scan(1,0,.1));auto second=scan(2,1,.1);second.receiver_position.reset();store.append(second);auto third=scan(3,0,.5);third.counts[19]=500;third.counts[32]=1500;store.append(third);store.append(packet());
    ...
    rejects([&]{store.append(authorized_packet());},"decode opt-out enforced at persistence boundary");
...
void packet_only(const std::filesystem::path& directory) {
    ...
    {ovmesh::SessionStore store;store.create(path.string(),c,"synthetic-packet-only");store.append(fix());store.append(authorized_packet());ovmesh::Snapshot s;s.config=c;s.elapsed_seconds=1;store.update(s,true);}
    ovmesh::SessionStore store;store.open_readonly(path.string());const auto s=store.read();require(s.config.survey_span_hz==26000000&&!s.config.concentrators.scan_enabled&&s.concentrator_scans==0,"26 MHz range and packet-only source roundtrip");require(s.receptions.size()==1&&s.receptions[0].decoded.authorized&&s.receptions[0].concentrator,"authorized packet and hardware provenance remain associated");
    ovmesh::ReportOptions options;options.kind=ovmesh::ReportKind::AuthorizedContent;options.privacy.include_content=true;ovmesh::export_survey_report(store,(directory/"content.csv").string(),options);const auto content=contents(directory/"content.csv");require(content.find("SYNTHETIC CONCENTRATOR CONTENT")!=std::string::npos&&content.find("packet_rssi_dbm_uncalibrated")!=std::string::npos,"authorized-content report includes hardware provenance");
...
#endif
    const auto file=directory/"survey.sqlite";create(file);roundtrip(file,directory);packet_only(directory);unsigned i=0;
