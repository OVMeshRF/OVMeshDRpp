...
void make(const std::filesystem::path& p,bool compact){ovmesh::SessionStore writer;writer.create(p.string(),config(compact),"synthetic compact storage");
    ...
    reception.id=2;reception.header_valid=reception.crc_valid=true;reception.decoded.status=ovmesh::protocol::Status::decoded;reception.decoded.classification="likely Meshtastic";
    ovmesh::protocol::AuthorizedContent content;content.profile_id="synthetic compact profile";content.from=0x11111111;content.to=0xffffffff;content.port=1;content.content.kind="text";content.content.text="SYNTHETIC COMPACT RETENTION";reception.decoded.authorized=content;writer.append(reception);
