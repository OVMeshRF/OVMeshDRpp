...
4. Exclude local assistant instructions and scope/approval history, private audit notes, internal legal/research working papers, credentials, preferences, operational databases, raw RF captures, GPS logs/routes, private messages and device identifiers. Check examples, documentation, screenshots, image metadata and diagnostic files as well as source. A Git ignore rule does not protect tracked or force-added files.
