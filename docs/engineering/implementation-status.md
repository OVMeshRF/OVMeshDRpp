...
| Concentrator | Experimental one/two-board RAK5146 USB/LBT adapter with independent configured packet profiles and shared RSSI sweep range | US915, macOS/Linux worker; no Windows worker, IQ waterfall, continuous occupancy or blind packet discovery. See [RAK setup](../operations/rak5146.md) |
...
| Preferences | Non-secret receiver/source/display settings, recording folder and explicit opt-outs; preference format 7 reads formats 1–6 | Coordinates, messages, channel keys and historical results are not preference defaults; selected USB identities remain local |
...
| Concentrator recording | Schema 7 retains complete sampled RSSI histograms, configured packet metadata, GPS associations and per-board health | Separate sampled measurement semantics; no USB identities or unknown payloads in saved surveys |
