...
Preference format 5 stores GPS/recording switches and folder, GPS identity/baud, source, center/rate/span, LNA/VGA/amplifier, signed Offset, Compact/Detailed mode, discovery/decoding/Spectrum-only choices, typography and position-display mode. Offset is bounded to ±100,000 Hz, defaults to zero and is not a per-device calibration record. New preferences enable discovery and arm decoding, but contain zero keys; arming does not authorize unrelated channels or implement full-range payload dispatch. Spectrum only suppresses discovery and payload processing without erasing setup. Keys, precise coordinates, RF profile/key material, decoded content and export privacy choices remain excluded. Parsing is bounded to 32 KiB and rejects malformed/unknown layouts. Formats 1–4 remain readable; present opt-outs survive migration to format 5, and absent fields use their documented defaults. Private temporary creation and atomic replacement preserve settings. POSIX app-owned directories/files use 0700/0600; the Windows owner-only ACL implementation still needs native validation. Permissions/hex encoding are not encryption, and folder/device identifiers remain identifying local metadata.
