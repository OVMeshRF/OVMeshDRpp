...
void rak_html_report(const SessionStore& store,const Context& c,const std::function<void(std::string_view)>& output) {
    ...
    emit("<!doctype html><html lang=\"en\"><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><meta http-equiv='Content-Security-Policy' content=\""+html_escape(report_csp)+"\"><title>Concentrator RF survey</title><style>body{font:16px system-ui;max-width:1200px;margin:2em auto;padding:1em}table{border-collapse:collapse;width:100%}th,td{padding:.5em;border-bottom:1px solid #bbb;text-align:left}small{color:#444}</style><h1>Concentrator RF survey</h1>");
    emit(std::string("<style>")+report_print_css+"</style>"+report_controls(false));
    ...
    emit("</tbody></table><p>Empty selections have no recorded samples, not proven quiet airwaves. A last receiver fix describes one scan endpoint, not every sample in its frequency summary. The detailed CSV preserves all 33 histogram bins and per-scan intervals. No external map or network resource is used. The local PDF button opens the browser print dialog.</p></html>");
}
}
