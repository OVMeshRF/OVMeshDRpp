 ...
 *
*/
void mcu_transport_opened(int fd); /* New descriptor only; resets failure latch. */
int mcu_transport_failed(void);
/* Fixed diagnostic code: 0 none/other, 1 ACK header timeout, 2 ACK body,
   3 request timeout. No request bytes, device identity or payload is exposed. */
int mcu_transport_timeout_kind(void);
