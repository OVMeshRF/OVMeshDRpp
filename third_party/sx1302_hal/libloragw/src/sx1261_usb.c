...
int sx1261_usb_w(void *com_target, sx1261_op_code_t op_code, uint8_t *data, uint16_t size) {
    int usb_device;
    if (size > 249) return -1;
    uint16_t command_size = size + 6; /* 5 bytes: REQ metadata, 1 byte: op_code */
    ...
    /* Keep the HAL mode/flush API, but acknowledge each auxiliary write before
       sending the next. A scan tune previously combined eight operations into
       one 79-byte MCU payload; keep this path individually paced instead of
       reintroducing aggregate requests alongside bounded memory transfers. */
