...
int sx1302_agc_load_firmware(const uint8_t *firmware) {
    ...
    fprintf(stderr, "AGC volatile RAM upload begins: %u bytes, 32-byte chunks\n", (unsigned)MCU_FW_SIZE);
    if (lgw_mem_wb(AGC_MEM_ADDR, firmware, MCU_FW_SIZE) != LGW_REG_SUCCESS) {
        fprintf(stderr, "AGC upload failed; no readback or further requests attempted\n");
        return LGW_REG_ERROR;
    }

    /* Read back and check */
    if (lgw_mem_rb(AGC_MEM_ADDR, fw_check, MCU_FW_SIZE, false) != LGW_REG_SUCCESS) {
        fprintf(stderr, "AGC readback failed; comparison skipped\n");
        return LGW_REG_ERROR;
    }
...
int sx1302_arb_load_firmware(const uint8_t *firmware) {
    ...
    if (err != LGW_REG_SUCCESS) return LGW_REG_ERROR;
    fprintf(stderr, "ARB volatile RAM upload begins: %u bytes, 32-byte chunks\n", (unsigned)MCU_FW_SIZE);
    if (lgw_mem_wb(ARB_MEM_ADDR, &firmware[0], MCU_FW_SIZE) != LGW_REG_SUCCESS) {
        fprintf(stderr, "ARB upload failed; no readback or further requests attempted\n");
        return LGW_REG_ERROR;
    }

    /* Read back and check */
    if (lgw_mem_rb(ARB_MEM_ADDR, fw_check, MCU_FW_SIZE, false) != LGW_REG_SUCCESS) {
        fprintf(stderr, "ARB readback failed; comparison skipped\n");
        return LGW_REG_ERROR;
    }
    if (memcmp(firmware, fw_check, sizeof fw_check) != 0) {
...
int sx1302_send(lgw_radio_type_t radio_type, struct lgw_tx_gain_lut_s * tx_lut, bool lwan_public, struct lgw_conf_rxif_s * context_fsk, struct lgw_pkt_tx_s * pkt_data) {
    return LGW_REG_ERROR; /* Receive-only bench. */
    int err;
