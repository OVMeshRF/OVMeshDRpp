...
static int compare_pkt_darwin(void *ctx,const void *a,const void *b) { return compare_pkt_tmst(a,b,ctx); }
#endif
...
#if defined(__APPLE__)
    qsort_r(p, cpt, sizeof(p[0]), &counter_qsort_swap, compare_pkt_darwin);
#else
...
int lgw_send(struct lgw_pkt_tx_s * pkt_data) {
    return LGW_HAL_ERROR; /* Receive-only bench: no TX entry point. */
    int err;
    bool lbt_tx_allowed;
    /* performances variables */
    struct timeval tm;
