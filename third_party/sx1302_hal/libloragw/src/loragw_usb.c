...
int set_interface_attribs_linux(int fd, int speed) {
    ...
    tcflag_t hardware_flow = 0;
#ifdef CCTS_OFLOW
    hardware_flow |= CCTS_OFLOW;
#endif
#ifdef CRTS_IFLOW
    hardware_flow |= CRTS_IFLOW;
#endif
#ifdef CDTR_IFLOW
    hardware_flow |= CDTR_IFLOW;
#endif
#ifdef CDSR_OFLOW
    hardware_flow |= CDSR_OFLOW;
#endif
#ifdef CRTSCTS
    hardware_flow |= CRTSCTS;
#endif
    tcflag_t previous_hardware_flow = tty.c_cflag & hardware_flow;
    cfmakeraw(&tty);
    tty.c_cflag &= ~hardware_flow;
    ...
    if (tcgetattr(fd, &actual) != 0 || (actual.c_cflag & hardware_flow) ||
        (actual.c_iflag & (IXON | IXOFF | IXANY | ICRNL | INLCR | IGNCR | ISTRIP)) ||
        (actual.c_oflag & OPOST) || (actual.c_lflag & (ICANON | ECHO | ISIG))) {
        fprintf(stderr, "USB raw-mode verification failed\n");
        return LGW_USB_ERROR;
    }
    fprintf(stderr, "USB raw 8N1: iflag=0x%lx oflag=0x%lx cflag=0x%lx lflag=0x%lx hardware-flow-before=0x%lx after=0x%lx\n",
        (unsigned long)actual.c_iflag, (unsigned long)actual.c_oflag,
        (unsigned long)actual.c_cflag, (unsigned long)actual.c_lflag,
        (unsigned long)previous_hardware_flow, (unsigned long)(actual.c_cflag & hardware_flow));
...
int lgw_usb_open(const char * com_path, void **com_target_ptr) {
    ...
    CHECK_NULL(com_path);
    *com_target_ptr = NULL;
    if (strlen(com_path) >= sizeof portname) return LGW_USB_ERROR;
    ...
    /* open tty port */
    snprintf(portname, sizeof portname, "%s", com_path);
    fd = open(portname, O_RDWR | O_NOCTTY | O_NONBLOCK | O_CLOEXEC | O_NOFOLLOW);
    if (fd < 0) {
        printf("ERROR: failed to open COM port %s - %s\n", portname, strerror(errno));
    } else {
        struct stat device_status;
        if (fstat(fd, &device_status) != 0 || !S_ISCHR(device_status.st_mode) || ioctl(fd, TIOCEXCL) != 0) {
            close(fd); free(usb_device); return LGW_USB_ERROR;
        }
        mcu_transport_opened(fd);
        printf("INFO: Configuring TTY\n");
        x = set_interface_attribs_linux(fd, B115200);
        if (x != 0) {
            printf("ERROR: failed to configure COM port %s\n", portname);
            close(fd); free(usb_device); *com_target_ptr = NULL;
            return LGW_USB_ERROR;
        }
        ...
        printf("INFO: Setting TTY in bounded nonblocking mode\n");
        ...
        *com_target_ptr = (void*)usb_device;
        ...
        if (strncmp(gw_info.version + 1, mcu_version_string, sizeof mcu_version_string) != 0) {
            printf("ERROR: MCU version mismatch (expected:%s, got:%s); refusing initialization\n", mcu_version_string, gw_info.version);
            close(fd); free(usb_device); *com_target_ptr = NULL;
            return LGW_USB_ERROR;
        }
...
int lgw_usb_close(void *com_target) {
    ...
    /* A failed transaction may leave the bridge waiting for its body. */
    if (!mcu_transport_failed()) {
    ...
    if (x != 0) {
        ...
        fprintf(stderr, "USB link failed: closing descriptor without GPIO requests\n");
    }
...
uint16_t lgw_usb_chunk_size(void) {
    return 32; /* Entire MCU memory request remains below one 64-byte USB packet. */
}
