...
int sx1261_get_status(uint8_t * status) {
    ...
    buff[0] = 0x00;
    if (sx1261_reg_r(SX1261_GET_STATUS, buff, 1) != 0) return -1;
...
int sx1261_load_pram(void) {
    ...
    if (sx1261_reg_w(SX1261_SET_STANDBY, buff, 1) != 0) return -1;
...
int sx1261_spectral_scan_start(uint16_t nb_scan) {
    int err;
    uint8_t buff[9] = {0}; /* 66 bytes for spectral scan results + 2 bytes register address + 1 dummy byte for reading */
    /* performances variables */
    struct timeval tm;
...
int sx1261_spectral_scan_get_results(int8_t rssi_offset, int16_t * levels_dbm, uint16_t * results) {
    int err, i;
    uint8_t samples[66];
    uint8_t buff[27]; /* <=24 data bytes, two-byte register address, one dummy */
    /* performances variables */
    struct timeval tm;
    ...
    /* The caller waits for COMPLETED and starts no new scan during readout.
       Read the stable 66-byte bank in bounded, separately acknowledged chunks.
       Do not expose partial results when a later USB transaction fails. */
    for (size_t offset = 0; offset < sizeof samples; offset += 24) {
        size_t count = sizeof samples - offset;
        if (count > 24) count = 24;
        const uint16_t address = (uint16_t)(0x0401U + offset);
        memset(buff, 0, sizeof buff);
        buff[0] = (uint8_t)(address >> 8);
        buff[1] = (uint8_t)address;
        err = sx1261_reg_r(SX1261_READ_REGISTER, buff, (uint16_t)(count + 3));
        if (err != LGW_REG_SUCCESS) return LGW_REG_ERROR;
        memcpy(samples + offset, buff + 3, count);
    }
    ...
    for (i = 0; i < 32; i++) {
        ...
        results[i] = (uint16_t)((samples[i*2] << 8) | samples[i*2 + 1]);
    }
    /* The number of points measured BELOW the lower threshold */
    levels_dbm[32] = -31*4 + rssi_offset;
    results[32] = (uint16_t)((samples[32*2] << 8) | samples[32*2 + 1]);
