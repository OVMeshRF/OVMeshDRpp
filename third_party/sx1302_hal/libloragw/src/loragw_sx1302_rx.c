...
int rx_buffer_fetch(rx_buffer_t * self) {
    ...
    /* A failed FIFO query must not reuse stale lengths or packet counts. */
    rx_buffer_del(self);

    /* Check if there is data in the FIFO */
    if (lgw_reg_rb(SX1302_REG_RX_TOP_RX_BUFFER_NB_BYTES_MSB_RX_BUFFER_NB_BYTES, buff, sizeof buff) != LGW_REG_SUCCESS)
    ...
    if (self->buffer_size > sizeof self->buffer) {
        rx_buffer_del(self);
        return LGW_REG_ERROR;
    }
    ...
    if (self->buffer_size > 0) {
        ...
        while (idx < self->buffer_size) {
            const size_t remaining = (size_t)self->buffer_size - (size_t)idx;
            if (remaining < SX1302_PKT_HEAD_METADATA + SX1302_PKT_TAIL_METADATA)
                return rx_buffer_del(self);
            if ((self->buffer[idx] != SX1302_PKT_SYNCWORD_BYTE_0) || (self->buffer[idx + 1] != SX1302_PKT_SYNCWORD_BYTE_1)) {
            ...
            if (remaining < SX1302_PKT_HEAD_METADATA + payload_len + SX1302_PKT_TAIL_METADATA)
                return rx_buffer_del(self);
            if (SX1302_PKT_NUM_TS_METRICS(self->buffer, idx + payload_len) > 127)
                return rx_buffer_del(self);
            next_pkt_idx =  SX1302_PKT_HEAD_METADATA +
                            payload_len +
                            SX1302_PKT_TAIL_METADATA +
                            (2 * SX1302_PKT_NUM_TS_METRICS(self->buffer, idx + payload_len));
            if (next_pkt_idx > remaining) return rx_buffer_del(self);

            /* Count only a complete, bounded frame. */
...
int rx_buffer_pop(rx_buffer_t * self, rx_packet_t * pkt) {
    ...
    /* Validate the fetched extent before indexing any variable packet metadata. */
    if (self->buffer_size > sizeof self->buffer || self->buffer_index < 0 ||
        self->buffer_index > self->buffer_size) return LGW_REG_ERROR;
    if (self->buffer_size - self->buffer_index < SX1302_PKT_HEAD_METADATA + SX1302_PKT_TAIL_METADATA)
    ...
    if (self->buffer_size - self->buffer_index < SX1302_PKT_HEAD_METADATA +
        pkt->rxbytenb_modem + SX1302_PKT_TAIL_METADATA) return LGW_REG_WARNING;
    ...
    if ((size_t)pkt->num_ts_metrics_stored * 2 > sizeof pkt->timestamp_avg ||
        (size_t)pkt->num_ts_metrics_stored * 2 > sizeof pkt->timestamp_stddev) return LGW_REG_WARNING;
    ...
    pkt->timestamp_cnt  = ((uint32_t)SX1302_PKT_TIMESTAMP_7_0(self->buffer, self->buffer_index + pkt->rxbytenb_modem) <<  0) & 0x000000FFU;
    pkt->timestamp_cnt |= ((uint32_t)SX1302_PKT_TIMESTAMP_15_8(self->buffer, self->buffer_index + pkt->rxbytenb_modem) <<  8) & 0x0000FF00U;
    pkt->timestamp_cnt |= ((uint32_t)SX1302_PKT_TIMESTAMP_23_16(self->buffer, self->buffer_index + pkt->rxbytenb_modem) << 16) & 0x00FF0000U;
    pkt->timestamp_cnt |= ((uint32_t)SX1302_PKT_TIMESTAMP_31_24(self->buffer, self->buffer_index + pkt->rxbytenb_modem) << 24) & 0xFF000000U;
...
void rx_buffer_dump(FILE * file, uint16_t start_addr, uint16_t end_addr) {
    return; /* No packet dumps in bench logs. */
    int i;
