 ...
 *
 * Modified by OVMeshDRpp contributors on 2026-09-10:
 * validate async allocations and transfer lengths, clean partial resources,
 * and reject R820T/R828D tuning success without PLL lock.
 *
...
int r820t_set_freq(void *dev, uint32_t freq) {
	rtlsdr_dev_t* devt = (rtlsdr_dev_t*)dev;
	int r = r82xx_set_freq(&devt->r82xx_p, freq);
	/* OVMesh: upstream can return zero without PLL lock. Do not cache a
	 * successful RF tune or start a survey from that unlocked state. */
	if (r >= 0 && !devt->r82xx_p.has_lock)
		return -EIO;
	return r;
}
...
static int _rtlsdr_alloc_async_buffers(rtlsdr_dev_t *dev)
{
	...
	if (!dev->xfer) {
		/* OVMesh: zero-initialize partial allocations for safe failure cleanup. */
			  ...
			  uint32_t buf_num, uint32_t buf_len)
{
	...
	/* OVMesh: libusb's transfer length is signed, although this API is unsigned. */
	if (buf_len > INT_MAX)
		return -EINVAL;
