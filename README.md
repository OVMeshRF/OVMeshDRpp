...
**Binary packaging is in progress; these candidates are not published downloads.** An Apple Silicon app/DMG has passed packaged synthetic launch checks, with Developer ID signing and notarization pending. Ubuntu 24.04 amd64 and arm64 builds have passed their offline test checks; package acceptance is tracked separately. Windows x64 preparation is underway, without a validated executable or ZIP yet. See [package status and platform limits](docs/operations/deployment.md#platform-and-distribution-limits).
